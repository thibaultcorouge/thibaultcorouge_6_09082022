...
exports.login = (req, res, next) => {
    User.findOne({ email: req.body.email}) // mongoose method to compare e-mail with the database if there's not e-mail user stored = error if there's keep going.
        .then(user => { 
            if(!user) {
            ...
            .then(valid => {
                ...
                res.status(200).json({
                    ...
                    token: jwt.sign( // Sign : jsonwebtoken function to encode a new token with a payload( userId ) 
                        { userId: user._id},
                        'RANDOM_TOKEN_SECRET', // random_token_secret to encode the token(this one is not enough complex for produtcion)
